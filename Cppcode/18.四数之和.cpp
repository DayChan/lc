...
public:
    vector<vector<int>> fourSum(vector<int>& nums, int target) {
        ...
        for (auto iter = nums.begin(); iter != nums.end(); iter++)
        {
            ...
            {
                if (iter2 != iter + 1 && *(iter2-1) == *(iter2))
                {
                    continue;
                }
                auto head_ptr = iter2 + 1;
                auto tail_ptr = nums.end() - 1;
                while (head_ptr < tail_ptr)
                {
                    if (*iter + *iter2 + *head_ptr + *tail_ptr < target)
                    {
                        do
                        {
                            head_ptr++;
                    ...
                    else if (*iter + *iter2 + *head_ptr + *tail_ptr == target)
                    {
                        res.push_back(vector<int> ({*iter, *iter2, *head_ptr, *tail_ptr}));
                        do
                        {
                            head_ptr++;
                        } while (head_ptr != nums.end() && (*head_ptr == *(head_ptr - 1)));
